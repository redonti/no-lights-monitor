...
// 0.036 ≈ ~8 km tip-to-tip at Ukraine's latitude.
let HEX_SIZE = 0.036;
const LAT_CORR = Math.cos(48.5 * Math.PI / 180); // ~0.661 for Ukraine (~48°N)

// Grid anchor — hex (q=0, r=0) is centred here, keeping q/r values small and stable.
const ANCHOR_LAT = 48.0;
const ANCHOR_LNG = 32.0;
...
function latlngToAxial(lat, lng) {
  const x = (lng - ANCHOR_LNG) * LAT_CORR;
  const y = (lat - ANCHOR_LAT);
...
function axialToLatLng(q, r) {
  return {
    lat: ANCHOR_LAT + HEX_SIZE * (3 / 2 * r),
    lng: ANCHOR_LNG + HEX_SIZE * (Math.sqrt(3) * q + Math.sqrt(3) / 2 * r) / LAT_CORR,
  };
}
...
// Choropleth color: green (0%) → amber (20%) → red (50%) → deep red (100%).
function choroColor(offlinePct) {
  if (offlinePct <= 0.2) {
    return rgbHex(lerpColor([74, 222, 128], [251, 191, 36], offlinePct / 0.2)); // green-400 → amber-300
  }
  if (offlinePct <= 0.5) {
    return rgbHex(lerpColor([251, 191, 36], [220, 38, 38], (offlinePct - 0.2) / 0.3)); // amber-300 → red-600
  }
  return rgbHex(lerpColor([220, 38, 38], [127, 29, 29], (offlinePct - 0.5) / 0.5)); // red-600 → red-900
}
...
function buildChoropleth() {
  ...
  for (const key in cells) {
    ...
    hex.bindTooltip(
      `<strong>${onPct}%</strong> зі світлом<br><span style="color:#78716c;font-size:0.85em;">${c.total - c.offline} з ${c.total} моніторів</span>`,
      { sticky: true },
    );
...
const HEX_LOG_MIN = Math.log(0.5);
const HEX_LOG_MAX = Math.log(10);
...
function applyHexSize(sliderVal) {
  const km = sliderToKm(parseInt(sliderVal, 10));
  HEX_SIZE = km / (2 * 111);
  const label = formatHexSize(km);
  document.getElementById('hex-size-label').textContent = label;
  document.getElementById('choro-size-label').textContent = label;
}

document.getElementById('hex-size').addEventListener('input', function () {
  applyHexSize(this.value);
...
(function restoreControls() {
  ...
  if (savedHexSize !== null) {
    document.getElementById('hex-size').value = savedHexSize;
    applyHexSize(savedHexSize);
  }
})();
